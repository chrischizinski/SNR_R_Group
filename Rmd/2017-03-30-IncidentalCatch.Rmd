...
Start the loop.  **NOTE**:  I changed the way the loop was run from the psuedo code by nesting iterations within tows.  
...
s_stor <- data.frame(Ntows =1:Ntows, s = NA, iter = iter)

  for(i in 3:Ntows){
    s = 0
    print(paste("Ntows = ",i,sep = ""))
    for(j in 1:iter){
      rand_uni<-runif(i)  #generate j number of random uniform variables
      ...
      S_j2 = var(catch)
      Range_j = 2*(sqrt(S_j2)/sqrt(i))*tq # Step 4 in the psuedo code
